...
const SECTIONS = [
  ...
  },
  {
    ...
    measures:
      'A validated five-factor personality profile: fifteen primary factors, each reported as a position on its own scale, aggregated into five global dimensions. Positions, not bands, and no sten: a sten is a comparison with a reference sample, no reference sample is documented for our band boundaries, and the band table itself is uneven (widths from 2 to 8 raw points on the first factor alone). Each factor is shown as a percentage of its own scale, against your own profile average.',
    scoring:
      '130 Likert items (120 personality + 10 social-desirability checks). Reverse-keyed items inverted, factor raw scores converted to stens via the frozen band table, global dimensions computed as weighted factor composites. The sten is computed and stored — it is the raw material for rebuilding the table — but it is no longer shown to a reader anywhere except inside the equation for a global dimension, where the arithmetic cannot be checked without it. A factor is named as one of the loudest in a profile only where it sits at least 20 points of its own scale from that profile\u2019s own average: derived from an assumed scale SD of 15 points and a pessimistic reliability of 0.70, then rounded up. Absolute, so an even profile names nothing at all. Two validity indices — social desirability and central tendency — are reported, never hidden.',
    weaknesses: [
  ...
  },
  {
    ...
    scoring:
      '140 Likert items, roughly a third reverse-keyed. Sub-dimension and domain scores are straight means on the 1–5 scale. The band words this instrument used to carry — High, Moderate, Developing, at cut-offs of 4.0 and 3.0 — have been withdrawn: a band word implies a standard, and no reference sample is documented for those cut-offs. A strongest or weakest domain is named only where it sits at least 0.8 clear of the domain next to it on that scale (derived at 0.765 and rounded up, never down). Where nothing clears that, the four individual figures are not shown either — four values in descending order rank them however the sentences are worded — and what is shown instead is the single band all four fall in, the widest gap between any two, and the floor they failed to clear. The fourteen sub-dimensions are named and not scored at all: shorter scales mean lower reliability and a larger floor than the domains have already failed, and fourteen numbers on a page are a ranking whatever order they are printed in.',
    weaknesses: [
