...
def _by(anx, avo, c_sten, a_sten, confidence="high"):
    return {
        "MI-AS-36": {
        ...
        "personality": {
            # Filler factors so the profile has a mean to be displaced FROM. Personality
            # displacement is within-profile — its absolute sten position is the paused
            # norm-referenced claim, and the frozen band table keeps real answers inside stens
            # 4-8 anyway, so an absolute test would never fire for anyone.
            "factor_scores": {"C": {"sten": c_sten, "name": "Emotional Stability"},
                              "A": {"sten": a_sten, "name": "Warmth"},
                              **{k: {"sten": 5, "name": k} for k in ("E", "F", "G", "H", "I")}},
...
def test_personality_displacement_is_within_profile_not_absolute():
    """A flat profile is not displaced however high it sits — and a sten of 6 in a profile
    averaging 5 is displaced when a sten of 9 in a profile of 9s is not."""
    flat = {(r["construct"], r["instrument"]): r for r in _readings(_by(4, 4, 9, 9))}
    flat_pers = flat[("steadiness", "personality")]
    by_flat = _by(4, 4, 9, 9)
    by_flat["personality"]["factor_scores"] = {
        "C": {"sten": 9, "name": "Emotional Stability"}, "A": {"sten": 9, "name": "Warmth"}}
    ...
    assert abs(flat_pers["displacement"]) >= 1.0  # 9 against a profile mean near 6
...
def test_the_closeness_dimensions_are_oriented_at_source():
    ...
    readings = {(r["construct"], r["instrument"]): r for r in _readings(_by(7, 7, 5, 5))}
    ...
    settled = {(r["construct"], r["instrument"]): r for r in _readings(_by(1, 1, 5, 5))}
...
def test_two_mid_scale_readings_are_not_convergence():
    ...
    out = build_convergences(_by(4.0, 4.0, 6, 6))
...
def test_no_superlative_survives_anywhere():
    ...
    everything = build_convergences(_by(1.5, 1.5, 9, 9)) + build_convergences(_by(4, 4, 6, 6))
...
def test_a_genuine_agreement_still_fires():
    ...
    out = build_convergences(_by(1.5, 1.5, 9, 9))
    ...
    assert "1.5" in body and "9" in body  # both values quoted
...
def test_an_agreement_inherits_the_weaker_input():
    ...
    out = build_convergences(_by(1.5, 1.5, 9, 9, confidence="low"))
...
def test_an_opposite_facing_pair_is_a_tension():
    ...
    stays out of reach."""
    out = build_tensions(_by(4.0, 6.5, 6, 9), [])
...
def test_one_displaced_and_one_at_the_middle_is_neither():
    ...
    out = build_convergences(_by(1.2, 4.0, 6, 6))
...
def test_the_summary_does_not_claim_a_driver_from_a_low_confidence_middle():
    ...
    by = _by(4.2, 3.7, 6, 6, confidence="low")
    by["essential"] = {
