...
| `DISPLAY_VERSION` | `disp-1.5.1` | position mapping · `1.0.0` the population layer, `1.1.0` the norms pause, `1.2.0` Everyday joins the combined reading, `1.2.1` the last sten leaks out of the reader-facing copy, `1.3.0` the Essential tie state names both where the top two sit inside the margin, `1.3.1` the tie note is labelled per lens and printed once, `1.3.2` a tied reading voices both archetype descriptions, not just the first-named, `1.3.3` the loudest-traits heading counts what actually cleared the floor and a single-named profile gets its own choosing point, `1.3.4` the one-factor partial line reads "One factor sits", `1.4.0` the reportable floors — convergence needs displacement, EI naming needs a minimum reportable difference, the social-desirability cut moves above chance, speeding is per item, and the Delta reports its elevation, `1.5.0` no sten reaches a reader anywhere and both reportable floors are rounded up rather than to the nearest value: the Personality layer is percent-of-scale against the reader's own profile average on an absolute 20-point floor, the EI floor is 0.8, and where no two EI domains clear it the individual figures are replaced by one shared band plus the observed spread and the floor, `1.5.1` the not-for-display marker on the sten is attached at read time so a payload scored before 1.5.0 carries the warning too |
